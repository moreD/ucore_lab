...
trap_dispatch(struct trapframe *tf) {
    ...
    case IRQ_OFFSET + IRQ_KBD:
        ...
        cprintf("kbd [%03d] %c\n", c, c);
        break;
    //LAB1 CHALLENGE 1 : 2013011407 you should modify below codes.
    case T_SWITCH_TOU:
		tf->tf_cs = USER_CS;
    ...
    case T_SWITCH_TOK:
		...
		tf->tf_eflags &= ~(3 << 12);
		memmove((uint32_t*)tf + 2, tf, sizeof(struct trapframe) - 8);
		*((uint32_t*)tf - 1) = (uint32_t)((uint32_t*)tf + 2);
        break;
