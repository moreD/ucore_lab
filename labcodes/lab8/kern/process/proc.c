...
alloc_proc(void) {
     ...
     */
     //LAB5 2013011407 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2013011407 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2013011407 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2013011407
    //LAB8:EXERCISE2 2013011407  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    */
	if ((proc = alloc_proc()) == NULL) {
		cprintf("bad fork, allocating proc\n");
	...
	if (setup_kstack(proc) != 0) {
		cprintf("bad fork, setting up kstack\n");
...
load_icode(int fd, int argc, char **kargv) {
	...
	uint32_t segno, offset = elf->e_phoff;
    uint32_t vm_flags, perm;
	for (segno = 0; segno < elf->e_phnum; ++segno) {
		if ((ret = load_icode_read(fd, ph, sizeof(struct proghdr), offset)) != 0)
		...
		uint32_t ph_offset = ph->p_offset;
        while (start < end) {
	...
	uint32_t length = 0, top = USTACKTOP;
	int32_t i;
	for (i = 0; i < argc; ++i)
		length += strlen(kargv[i]) + 1;
	top -= length * sizeof(char);
	char ** args = top - argc * sizeof(char *);

	uint32_t pos = 0;
	for (i = 0; i < argc; ++i) {
		args[i] = strcpy((char*)top + pos, kargv[i]);
		pos += strlen(kargv[i]) + 1;
	}
	top -= argc * sizeof(char *) + sizeof(int);
	*(int *)top = argc;
